...
class PO():
    ...
    def set_predictions_from_predictions_with_variables(self, doCompare, begin, end, td = 10):
        if(not doCompare):
            self.predictions = self.prediction_with_variables(begin, end, td)
        else:
            #The question is do I use the replace, or multiply? 
            predictions2 = self.prediction_with_variables(begin, end, td+5).replace(1,2)
            predictions1 = self.prediction_with_variables(begin, end, td)
            self.predictions = predictions2.sub(predictions1)
